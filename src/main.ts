 ...
 */
function updateSectionTitles(lang: string) {
 ...
 * @param {string} lang - The code of the selected language (e.g., 'fr' for French, 'en' for English).
 */
function updateButtonStyles(lang: string) {
 ...
 * @returns {void}
 */
function main(lang: string): void {
  try {
    ...
    buttons.forEach(button => {
      button.addEventListener('click', () => {
        ...
        const selectedLang = (button as HTMLElement).innerText.toUpperCase() === 'EN' ? 'en' :
